...
# BEWARE: THIS IS THE GEOMETRIC MEAN VERSION OF THIS SCRIPT
...
# psych package has an integrated function for the geometric mean
library(psych)
...
**#MP deleted: filter(str_detect(effect, "Mort") \| str_detect(effect, "Immobil")),** since this dismisses intoxication, mobility, loss of equilibrium. Relevant effect types are filtered later filtered specifically for fish and daph
...
ratios_by_cas_df_reduced_preprocess %>% 
  # filter(cas %in% ratios_all_df$cas) %>% 
  # pivot_longer(cols = 12:16, names_to ="categorization_scheme", values_to = "categories") %>% 
  # filter(!is.na(categories)) %>% 
  # mutate(categories = str_replace_all(categories, "_", " ")) %>% 
  # mutate(categories = str_replace_all(categories, "a ch e", "AChE")) %>% 
  # mutate(categories = str_replace_all(categories, "irac moa other", "IRAC MoA, other")) %>% 
  # mutate(categorization_scheme = str_replace_all(categorization_scheme, "_moa_reduced", "")) %>% 
  # mutate(categorization_scheme = toupper(categorization_scheme)) %>% 
  ...
  # mutate(categories = str_replace_all(categories, "Moa", "MoA")) %>% 
...
sankey_data <-
  ratios_by_cas_df_reduced_preprocess %>% 
  # pivot_longer(cols = 23:27, names_to ="categorization_scheme", values_to = "categories") %>% 
  # filter(!is.na(categories)) %>% 
  # mutate(categories = str_replace_all(categories, "_", " ")) %>% 
  # mutate(categories = str_replace_all(categories, "a ch e", "AChE")) %>% 
  # mutate(categories = str_replace_all(categories, "irac moa other", "IRAC MoA, other")) %>% 
...
sankey_data %>% 
  # mutate(node = factor(node, levels=rev(node_order))) %>%
  ggplot(aes(x = x,
               next_x = next_x, 
               node = node, 
               next_node = next_node,
...
fish96_summarized_by_cas <- fish96 %>% 
  ...
  summarize(median_fish = median(effect_value, na.rm = TRUE),
            ...
            geomean_fish = geometric.mean(effect_value, na.rm = TRUE),
...
daph48_summarized_by_cas <- daph48 %>% 
  ...
  summarize(median_daphn = median(effect_value, na.rm = TRUE),
            ...
            geomean_daphn = geometric.mean(effect_value, na.rm = TRUE),
...
and labeled compounds with ratio above 100 and lm equation (similar to Belanger)

```{r}

line_labels <- tibble(geomean_fish = c(100000,100000,100000),
                      geomean_daphn = c(100000,1000000,10000000),
                      label = c(">1", ">10", ">100"))
...
pointplot_data <-
  ...
  mutate(geomean_ratio = (geomean_daphn/geomean_fish)) %>%
  filter(!is.na(geomean_ratio)) %>% 
  mutate(label = ifelse(geomean_ratio >100, chemical_name, NA_character_),
         color_category = case_when(
           geomean_ratio < 10 ~ "Below 10",
           geomean_ratio >= 10 & geomean_ratio <= 100 ~ "10 to 100",
           geomean_ratio > 100 ~ "Above 100"))
...
pointplot_data %>% 
  ggplot(aes(x=geomean_fish, y=geomean_daphn))+
  ...
  guides(color = "none")+
  labs(x = "geomean Fish LC50, mg/L",
       y = "geomean Daphnids EC50, mg/L")+
  geom_label(data = line_labels, aes(x=geomean_fish, y=geomean_daphn, label = label), linetype = c(1,2,3))+
  stat_poly_line(se = FALSE)+
  stat_poly_eq(mapping = use_label(labels = c("eq", "R2")))
  


ggsave(plot = last_plot(), here::here("figures", "GEOMEAN-Fish96 vs. Daphn48 by cas.png"), width = 140, height = 140, unit = "mm", dpi = 600)
...
ratios_by_cas_overview_plot <-
  ...
  labs(x="Geomean ratio",
       y="Number of chemicals")

print(ratios_by_cas_overview_plot)

ggsave(plot = ratios_by_cas_overview_plot, here::here("figures", "GEOMEAN-ratios_by_cas_overview_plot.png"))
```
...
effect_values_by_cas_overview <- ratios_by_cas_df %>% 
  pivot_longer(cols = c("geomean_fish", "geomean_daphn"), names_to ="tax_group", values_to = "effect_value") %>% 
  mutate(tax_group =str_replace_all(tax_group, "geomean_fish", "Fish"),
         tax_group =str_replace_all(tax_group, "geomean_daphn", "Daphnids")) %>% 
  ...
  scale_fill_manual(values=c("Fish" = "#218380",
                             "Daphnids" = "#ffbc42"))+
  theme(legend.position = "none")+
  labs(y="Taxonomic group", x = "EC50 or LC50 [mg/L], geomean by cas")

print(effect_values_by_cas_overview) 


ggsave(plot = effect_values_by_cas_overview, here::here("figures", "GEOMEAN-effect_values_by_cas_overview.png"))
...
ggsave(plot = last_plot(), here::here("figures", "GEOMEAN-Patchwork-dataset_overview_figure.png"), dpi = 600, units = "mm", width = 250, height = 280, device = "png")
```
...
(# MP added Caffeine, Nicotine was missing in the script of CS)
...
(# MP: the following IRAC categories were missing via the script of CS,, which contain 1 chemical each: 1) gaba gated chloride channel blockers phenylpyrazoles fiproles, 2) octopamine receptor agonists, 3) voltage dependent sodium channel blockers oxadidiazines semicarbazones)
...
write_xlsx(ratios_by_cas_df_reduced, here::here("tables", "GEOMEAN-ratios_by_cas_df_reduced.xlsx"))
```
...
!!! This is the correct way to calculate the ratio brackets !!!
...
ratios_by_cas_df_reduced_overview_summary <-
  ...
  mutate(ratio_bracket = case_when(
    geomean_ratio <= 1 ~ "<=1",
    geomean_ratio > 1 & geomean_ratio <= 10 ~ ">1",
    geomean_ratio > 10 & geomean_ratio <= 100 ~ ">10",
    geomean_ratio > 100 ~ ">100")) %>% 
...
write_xlsx(ratios_by_cas_df_reduced_overview_summary, here::here("tables", "GEOMEAN-ratios_by_cas_df_reduced_overview_summary.xlsx"))
```
...
heatmap_table_ratios_brackets <-
  ...
  #this uses the same reorder-categories as the violin-plot
  # mutate(categories = factor(categories, levels=violin_reorder_subcategories)) %>%
  ...
  theme(strip.background = element_rect(color="black", fill="#FFFFFF", linewidth=1.5, linetype=0),
    ...
    #   str_detect(categorization_scheme, "ASTER") ~ scale_y_discrete(breaks = c("Non-Neurotoxic",
    #                                                                  "No Categorization",
    #                                                                  "Carbamate Mediated AChE Inhibition",
    #                                                                  "Neurotoxicant: Cyclodiene-Type",
    #                                                                  "Neurotoxicant: Nicotine",
    ...
    #                                                                  "Neurotoxicant: Pyrethroid",
    #                                                                  "Op Mediated AChE Inhibition")),
    #   str_detect(categorization_scheme, "EFSA") ~ scale_y_discrete(breaks = c("No Categorization",
    #                                                                 "Neurotoxic"),
    #                                                      labels = c("No Categorization",
    #                                                                 "Neurotoxic")),
    #   str_detect(categorization_scheme, "IRAC") ~ scale_y_discrete(breaks = c("No Categorization",
    #                                                                 "(nAChr) Interaction",
    #                                                                 "AChE Inhibitors",
    #                                                                 "Gaba Gated Clc Blockers Cyclodiene Organochlorines",
    #                                                                 "Organophosphates",
    ...
    #                                                                 "Sodium Channel Modulators",
    #                                                                 "IRAC Moa Other")),
    #   str_detect(categorization_scheme, "KRAMER") ~ scale_y_discrete(breaks = c("No Categorization",
    #                                                                   "Non-Neurotoxic",
    #                                                                   "Neuroactive"),
    #                                                        labels = c("No Categorization",
    #                                                                   "Non-Neurotoxic",
    #                                                                   "Neuroactive")),
    #  str_detect(categorization_scheme, "TEST") ~ scale_y_discrete(breaks = c("No Categorization",
    #                                                                 "Non-Neurotoxic",
    #                                                                 "AChE Inhibition",
    #                                                                 "Neurotoxicity"),
    #                                                      labels = c("No Categorization",
...
ggsave(plot = last_plot(), filename = "GEOMEAN-Heatmap table ratio distribution across categories.png", path = here::here("figures"), height = 180, width = 180, units = "mm", device = "png")

ggsave(plot = last_plot(), filename = "GEOMEAN-Heatmap table ratio distribution across categories.pdf", path = here::here("figures"), height = 180, width = 180, units = "mm", device = "pdf")
```
...
cyclodiene_subset_fullviolin <- ratios_by_cas_df_reduced_preprocess_violin %>% 
  ...
  # mutate(subcategory = factor(subcategory, levels=rev(violin_reorder_subcategories)))
...
main_violin_plot <- ratios_by_cas_df_reduced_preprocess_violin %>%
  # mutate(subcategory = replace_na(subcategory, "No Categorization")) %>% 
  # mutate(subcategory = factor(subcategory, levels=rev(violin_reorder_subcategories))) %>% 
  filter(!cas %in% cyclodiene_subset_fullviolin$cas) %>% 
  ggplot(aes(y = subcategory, x= geomean_ratio))+
...
ggsave(plot = last_plot(), filename = "GEOMEAN-Violin_plot_by_cas.png", path = here::here("figures"), height = 300, width = 250, units = "mm", device = "png")
```
...
ratios_by_cas_df_reduced_preprocess_violin %>% 
  ...
  # filter(str_detect(chemical_name, "Lind")) %>% 
  # filter(geomean_ratio >= 10) %>% 
...
ratios_by_cas_df_reduced_preprocess %>%
  filter(geomean_ratio >= 10) %>% 
  select(chemical_name, cas, geomean_ratio, cv_fish, n_fish, cv_daphn, n_daphn, categorization_scheme, subcategory) %>% 
  pivot_wider(names_from = categorization_scheme, values_from = subcategory, values_fill = "No categorization") %>% 
  unique() %>% 
  write_xlsx(path = here::here("output", "GEOMEAN-chemicals_with_ratio_over10.xlsx"))
...
```{r}
ratios_by_cas_df_reduced_preprocess %>% 
  select(cas, chemical_name, geomean_ratio, categorization_scheme, subcategory) %>% 
...
ratios_by_cas_df_ext <- fish96_summarized_by_cas_ext %>%
  ...
  mutate(geomean_ratio = (median_daphn/median_fish)) %>%
  # filter(!is.na(geomean_ratio)) %>%
  ungroup() %>%
  left_join(substances_full, by = "cas")
...
selected_ratios <-
  ...
  select(chemical_name, cas, median_fish, cv_fish, n_fish, median_daphn, cv_daphn, n_daphn, geomean_ratio) %>% 
  distinct()
...
test_filtered_FIAA <- test %>% 
  ...
  filter(test_statistic == "LC50" | test_statistic == "EC50") %>% 
  filter(str_detect(source, "embryo|larva|hatch|egg", negate = TRUE)) %>% 
  filter(!str_detect(effect, "(?i)embryo|(?i)fetus"))%>% # added by MP
  select(-duration, -duration_days) %>% 
  separate(chemical_name, into = c("chemical_name", NA), sep = ";") %>%
  select(-effect_is_5x_above_water_solubility, -source, -version, -reported_chemical_name)
...
pointplot_data_FIAA <-
  ...
  left_join(unique(select(substances_full, cas, chemical_name)), by = "cas") %>% 
  mutate(geomean_ratio = (median_daphn/median_amph)) %>%
  filter(!is.na(geomean_ratio)) %>% 
  mutate(label = ifelse(geomean_ratio >10, chemical_name, NA_character_),
         color_category = case_when(
           geomean_ratio < 10 ~ "Below 10",
           geomean_ratio >= 10 & geomean_ratio <= 100 ~ "10 to 100",
           geomean_ratio > 100 ~ "Above 100"))
...
write_xlsx(here::here("tables", "geomean_ratio_brackets_percents_daphnids-amphibians.xlsx"))
...
fish96 %>% 
  ...
  mutate(geomean_ratio = effect_value_daph/effect_value_fish) %>% 
  select(cas, geomean_ratio) %>% 
...
rbind(approach_comparison_all_to_all) %>% 
  mutate(approach = factor(approach, levels=rev(approach_order))) %>% 
  ggplot(aes(y=approach, x = geomean_ratio, fill = approach))+
...
```{r}
ratios_by_cas_df %>% 
  filter(str_detect(chemical_name, "Roten")) 
```
